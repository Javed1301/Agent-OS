...
Invoked by the Node.js PythonSubprocessAdapter:
  python runner.py --mode <agentId> --inputs <json> --run-dir <absPath>

The --run-dir is an isolated per-execution folder:
  data/executions/<execId>/
  ├── input.json      (written by Node gateway before runner starts)
  ├── output.json     (written by Node gateway)
  ├── logs.txt        (appended by Node gateway)
  └── artifacts/      (output files from this runner are moved here)
...
Legacy --output-dir is still accepted for backward compatibility.
"""
...
# ---------------------------------------------------------------------------
# Mode wrappers for workflow agents
# ---------------------------------------------------------------------------

def run_planner_agent(inputs: dict[str, Any], output_dir: str, cwd: str) -> None:
    emit("log", "Loading Planner Agent...")
    sys.path.insert(0, cwd)
    import planner  # noqa: PLC0415
    result = planner.run(inputs, output_dir)
    emit("result", result)


def run_code_generator_agent(inputs: dict[str, Any], output_dir: str, cwd: str) -> None:
    emit("log", "Loading Code Generator Agent...")
    sys.path.insert(0, cwd)
    import code_generator  # noqa: PLC0415
    result = code_generator.run(inputs, output_dir)
    emit("result", result)


def run_reviewer_agent(inputs: dict[str, Any], output_dir: str, cwd: str) -> None:
    emit("log", "Loading Reviewer Agent...")
    sys.path.insert(0, cwd)
    import reviewer  # noqa: PLC0415
    result = reviewer.run(inputs, output_dir)
    emit("result", result)
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Agent Workspace runner — mode-dispatched")
    ...
    # --run-dir is the new name; --output-dir kept for backward compatibility
    parser.add_argument("--run-dir", dest="run_dir", default=None,
                        help="Absolute path to per-execution isolated run directory")
    parser.add_argument("--output-dir", dest="output_dir", default=None,
                        help="Legacy: absolute path to per-execution output directory")
    ...
    # Resolve the run directory (--run-dir preferred, --output-dir as fallback)
    run_dir: str = args.run_dir or args.output_dir or os.path.join(cwd, "_run")
    # Artifacts go in a subfolder of run_dir
    output_dir: str = os.path.join(run_dir, "artifacts")
    os.makedirs(output_dir, exist_ok=True)

    emit("status", "started")

    try:
        if args.mode == "hate-speech":
        ...
        elif args.mode == "planner-agent":
            run_planner_agent(inputs, output_dir, cwd)

        elif args.mode == "code-generator-agent":
            run_code_generator_agent(inputs, output_dir, cwd)

        elif args.mode == "reviewer-agent":
            run_reviewer_agent(inputs, output_dir, cwd)
