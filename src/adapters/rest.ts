 ...
 * RestApiAdapter
 *
 * Communicates with the AI Meeting Notes FastAPI service running independently
 * at localhost:8000. Does NOT spawn or manage the uvicorn process.
 *
 * Execution flow:
 ...
 * (FastAPI has no cancel endpoint — the background task runs to completion server-side.)
 */
export const restAdapter: Adapter = {
  start(ctx: AdapterContext): AdapterHandle {
    ...
    const baseUrl = (agent.configuration as { baseUrl?: string } | undefined)?.baseUrl
      ?? agent.entrypoint;
    ...
    async function run(): Promise<void> {
      try {
        ...
        // Kick off meeting notes generation
